...
import { initTelegram, haptic, showBackButton, backToHubOnResume } from '../../js/tg.js?v140';
import { submitScore, addPoints, getBest, clearState, settleAbandonedRun, oynanabilirMi } from '../../js/store.js?v140';
import { registerTexts, t, applyStaticTexts, locale, mhHtml } from '../../js/i18n-hook.js?v140';
import { SFX, soundToggleHtml, mountSoundToggle } from '../../js/audio.js?v140';
...
function tick() {
  ...
  if (yemVar) {
    ...
    if (eatenThisLevel >= FOODS_PER_LEVEL) {
      ...
      haptic.success();
      altinPatlama(hedef);
      // Yenen altin yem hucresi yilanin yeni kafasiyla ayni - food'u
      // temizlemezsek render() ayni hucreye hem 'head' hem 'food/gold'
      // sinifini birden ekliyor, gold::before kafayi tamamen kapliyordu.
      food = -1;
      render();
      return levelUp();
    }
