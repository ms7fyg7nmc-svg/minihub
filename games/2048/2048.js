...
const SIZE = 4;
const GAME_ID = '2048';
const POINTS_DIVISOR = 10; /* oyun skorunun kaci hub puani oldugu: 1000 skor = 100 puan */
...
function move(direction) {
  ...
  if (won && !wonShown) {
    wonShown = true;
    haptic.success();
    showOverlay('2048!', 'Harika. Devam edip daha yükseğe çıkabilirsin.', 'Devam et', hideOverlay);
  } else if (isGameOver()) {
    endGame();
  }
}
...
async function endGame() {
  ...
  const lines = [`Skorun: ${score.toLocaleString('tr-TR')}`];
  if (result.isRecord) lines.push('Yeni rekor!');
  if (earned > 0) lines.push(`+${earned.toLocaleString('tr-TR')} hub puanı kazandın.`);

  showOverlay('Oyun bitti', lines.join(' · '), 'Yeniden oyna', startNewGame);
}
